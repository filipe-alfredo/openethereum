...
if [ "$1" == "yes" ]; then \
    ...
    sed -e "s|0x0000000000000000000000000000000000000001|"${ACCOUNT_ADDRESS}"|g" -e "s|NETWORK_NAME|"${NETWORK_NAME}"|g" ${BASE_PATH}/config/genesis.json.example > ${BASE_PATH}/config/genesis.json && \
    sed -e "s|0x0000000000000000000000000000000000000001|"${ACCOUNT_ADDRESS}"|g" -e "s|FILEPATH_GENESIS|${FILEPATH_GENESIS}|g" -e "s|FILEPATH_ACCOUNT_PASSWORD|${FILEPATH_ACCOUNT_PASSWORD}|g" ${BASE_PATH}/config/config.toml.example  >> ${BASE_PATH}/config/config.toml && \
